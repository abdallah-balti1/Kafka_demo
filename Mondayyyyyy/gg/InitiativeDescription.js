                ...
                {renderCommiteeDate(
                  ...
                  nbCommitees === 1
                    ...
                    : (value => {
                        ...
                        // pre1 doit être avant pre2 (si 3 committees) ou avant committeeDate (si 2)
                        const nextDate = nbCommitees === 3
                          ? values.commiteeDates && values.commiteeDates[1]
                          : values.committeeDate;
                        if (nextDate && selectedDate >= new Date(nextDate)) {
                          return nbCommitees === 3
                            ? 'This date has to be before the second pre committee date'
                            : 'This date has to be before the committee date';
                        }
                        return undefined;
                      }),
                  nbCommitees === 1
                    ? initiative.initiativeGovernance.commitees?.length === statusData.currentCommiteeOrder || statusData.currentStatus?.is_final_status === true
                    : statusData.currentCommiteeOrder >= 1 || statusData.currentStatus?.is_final_status === true,
                )}
                ...
                {nbCommitees >= 2 && renderCommiteeDate(
                  ...
                  nbCommitees === 2
                    ...
                    : (value => {
                        if (!value) return intl.formatMessage({ id: 'ERROR_FIELD_REQUIRED' });
                        const selectedDate = new Date(value);
                        // pre2 doit être après pre1
                        const pre1 = values.commiteeDates && values.commiteeDates[0];
                        if (pre1 && selectedDate <= new Date(pre1)) {
                          return 'This date has to be after the first pre committee date';
                        }
                        // pre2 doit être avant committeeDate
                        if (values.committeeDate && selectedDate >= new Date(values.committeeDate)) {
                  ...
                  nbCommitees === 2
                    ? initiative.initiativeGovernance.commitees?.length === statusData.currentCommiteeOrder
                    : statusData.currentCommiteeOrder >= 2 || statusData.currentStatus?.is_final_status === true,
                )}
