...
function LinkedErrorModal({ error, title, onClose }) {
  ...
  return (
    <Style.ModalOverlay>
      <Style.ModalBox>
        <Style.ModalIconWrap><WarningIcon /></Style.ModalIconWrap>
        <Style.ModalTitle>{title}</Style.ModalTitle>
        <Style.ModalErrorMsg>{error.message}</Style.ModalErrorMsg>
        <Style.ModalHint>
          Please unlink or reassign all initiatives before deleting this item.
        </Style.ModalHint>
        <Style.ModalConfirmBtn onClick={onClose}>Got it</Style.ModalConfirmBtn>
      </Style.ModalBox>
    </Style.ModalOverlay>
  );
}
...
export default function GovernanceConfig({
  ...
  return (
    <Style.Container>
      {/* ── Error Modals ───────────────────────────────────────────────────── */}
      <LinkedErrorModal
        error={deleteLinkedError}
        title="Cannot delete governance"
        onClose={clearGovernanceError}
      />
      <LinkedErrorModal
        error={deleteCommiteeLinkedError}
        title="Cannot delete committee"
        onClose={clearGovernanceError}
      />
      <LinkedErrorModal
        error={deleteStatusLinkedError}
      ...
      <Style.Header>
        ...
        <Style.AddBtn onClick={() => setIsModalOpen(true)}>+ Add Governance</Style.AddBtn>
      </Style.Header>

      <Style.TableWrapper>
        <Style.TableHeader>
          <Style.Cell flex={2}>Governance</Style.Cell>
          <Style.Cell flex={2}>Committee</Style.Cell>
          <Style.Cell flex={3}>Status Initiative</Style.Cell>
          <Style.Cell flex={1} center>Final Status?</Style.Cell>
          <Style.Cell width={100} center>Actions</Style.Cell>
        </Style.TableHeader>
        ...
        {governances.map(governance => (
          <Style.GovernanceBlock key={governance.id}>
            <Style.GovRow>
              <Style.Cell flex={2}>
                <Style.ToggleBtn onClick={() => toggleGovernance(governance.id)}>
                ...
                {(governance.commitees || []).map(commitee => (
                  <Style.CommiteeBlock key={commitee.id}>
                    <Style.ComRow>
                      <Style.Cell flex={2} />
                      <Style.Cell flex={2}>
                        <Style.ToggleBtn onClick={() => toggleCommitee(commitee.id)}>
